...
def main():
	...
	temp_dir = "/media/sf_e_drive/temp"
	...
	for inFile in input_file_set:
		...
		if run_bwa:
			if args.start != 'bam':
				...
				#bwa and samtools
				subprocess.call([bwaCMD+" mem -t 6 -a \
	...
	if run_cnv:
		...
		call_cnv(exomedepth_bam_list, manta_bam_list, OutputFolder, cnv_panel_csv, AbsPathToGenome, exomeDepthCMD, mantaCMD, bam_suffix, temp_dir, OutputLog)
...
def call_cnv(exomedepth_bam_list, manta_bam_list, out_dir, panel_csv, genome, exomeDepthCMD, mantaCMD, bam_suffix, temp_dir, OutputLog):
