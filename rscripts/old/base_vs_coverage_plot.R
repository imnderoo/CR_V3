...
png(filename=fileName, width=960, height=672, pointsize=12)
	...
	files <- list.files(path=dirPath, pattern="*all_only.hist", full.names=T, recursive=TRUE)
	...
	for (file in files) 
	{
		...
		if(any(sampleTable[c("sample")] == covFileName))
		{
			...
			{
				myPlot <- myPlot + 
				geom_line(data = covTable, aes(x = coverage, y = percentTotal, colour = "Other Samples"), alpha = 0.2)
			
			}
			else
			{
				myPlot <- ggplot(covTable, aes(x = coverage, y = percentTotal, colour="Other Samples")) + 
				geom_line(alpha = 0.2) 
			}

			curMaxY = max(covTable["percentTotal"])
	...
	# Plot
	if(exists("myPlot")) {
		myPlot <- myPlot + geom_line(data = mainTable, aes(x = coverage, y = percentTotal, colour = sampleID), alpha = 1.0) +
