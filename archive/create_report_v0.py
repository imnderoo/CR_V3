...
def main():
	...
	#variants_db = os.path.join(args.resource_folder,"csv_db","variants.db")
	...
	for sample_id in sample_dict:
		...
		sample_vcf = os.path.join(align_folder,sample_id + ".vcf")
		...
		###get_coverage (align_folder, analysis_folder, sample_id, args.qscore, args.coverage, genelist, bed, coverage_rscript, coverage_output_suffix, bedtoolsCMD, rscriptCMD)
		...
		filter_av_field = "Chr,Start,End,Ref,Alt,Func.refGene,Gene.refGene,ExonicFunc.refGene,AAChange.refGene,avsnp144,esp6500siv2_all,1000g2015aug_all,ExAC_nontcga_AL,Zygosity,QUAL,FILTER,DP,AD,GQ,GT,SB"
		...
		report_qc_dict = {}
		###report_qc_dict = report_qc_miseq (args.run_folder, run_id, sample_id)
...
def get_coverage(in_folder, analysis_folder, sample_id, qscore, coverage, genelist, bed, rscript, coverage_output_suffix, bedtoolsCMD, rscriptCMD):
	...
	calcMetrics(histogramFile, coverage, genelist)
...
def report_qc_miseq(run_folder, run_id, sample_id):
	...
	report_qc_dict['yield_total']= chipResultsSummary.findAll("yield")[0].string
	...
	report_qc_dict['cluster_density'] = str(tile_qc.mean_cluster_density_pf)
	report_qc_dict['cluster_pf'] = str(tile_qc.percent_pf_clusters)
	report_qc_dict['reads_pf']	= str(index_qc.total_ix_reads_pf)
	report_qc_dict['pct_gt_q30'] = str(sum(q30List) / len(q30List))
