...
def main():
	...
	call_cnv(bam_list, OutputFolder, cnv_panel_csv, AbsPathToGenome, exomeDepthCMD, mantaCMD, bam_suffix)
		
	end_time = time.asctime( time.localtime(time.time()) )
	print ("\nStart time: " + start_time + "\n")
	print ("End time: " + end_time + "\n")

def call_cnv(bam_list, out_dir, panel_csv, genome, exomeDepthCMD, mantaCMD, bam_suffix):

	cnv_out = os.path.join(out_dir,"cnv")

	# Create bamlist file
	bam_list_file = os.path.join(cnv_out,"bamlist.txt")
	...
	manta_target ="--region chr13 --region chr17 --region chr3 --region chr2 --region chr7 --region chr19 --region chr10 --region chr16 --region chr22 --region chr5 --region chr1 --region chr11"
