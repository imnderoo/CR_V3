...
def main():
	...
	parser.add_argument('out_folder', help = 'Path to Analysis Out Folder For Reports') 
	...
	if not os.path.isdir(analysis_folder):
		...
		os.makedirs(os.path.join(analysis_folder,"temp"))
		os.makedirs(os.path.join(analysis_folder,"csv"))
		os.makedirs(os.path.join(analysis_folder,"coverage_plots"))
	...
	requisite_flag = checkRequisites(args.run_folder, align_folder)
	...
	for sample_id in sample_dict:
		...
		#sample_id # should have been the same as the sample_sheet
						
		print ("\tFiltering by QScore and removing duplicates...")
		...
		[failed_exons_list, gene_exons_dict] = get_coverage (align_folder, analysis_folder, sample_id, args.qscore, args.coverage, genelist, bed, coverage_rscript, coverage_output_suffix, bedtoolsCMD, rscriptCMD)
		...
		av_file = os.path.join(align_folder, sample_id + av_output_suffix + ".txt")
		...
		# def filter_av(av_in, av_out, genelist, bed, av_fields, variants_db):
		#filter_av (flatten_av_file, filter_av_file, genelist, bed, filter_av_field, variants_db) 

		print ("\tCalculating QC metrics for " + sample_id + "...")
		
			
		# Compare count of variants reported (Failed + QC) vs. variants called.
		
		# Consolidate RunQC stats

		report_qc_dict[sample_id] = report_qc_miseq (args.run_folder, run_id, sample_id)
		
		#sample_vcf = glob.glob(align_folder + "/" + sample_id + raw_vcf_suffix + ".vcf")
	...
	for sample_id in sample_dict:
		...
		#for file in glob.glob(os.path.join(analysis_folder,"coverage","other_plots") + "/" + sample_id + "*.png"):	# Is this necessary? Is it debug only?
			#shutil.copy (file, os.path.join(args.out_folder, run_id, "coverage_plots", sample_id)

		# Create the PDF Reports
		print ("\tCreating PDFs for " + sample_id + "...")
		
		for file in glob.glob(analysis_folder + "/*.tmp"):	
			shutil.move (file, os.path.join(analysis_folder, "temp"))
		
		for file in glob.glob(analysis_folder + "/*.csv"):
			shutil.move (file, os.path.join(analysis_folder, "csv"))
		
		# Compare count of variants reported (Failed + QC) vs. variants called and rename the final PDF report if not equal
	
		print ("\tReport for " + sample_id +  " created.")
	
		shutil.copy(os.path.join(args.resource_folder,"tools","custom","ngs_toolkit","helpers","copy_reports_to_case_folders.vbs"), os.path.join(args.out_folder, run_id, "Reports"))
		shutil.copy(os.path.join(args.resource_folder,"tools","custom","ngs_toolkit","helpers","upload_ngs_results_to_access.vbs"), os.path.join(args.out_folder, run_id))
		shutil.copy(os.path.join(args.resource_folder,"tools","custom","ngs_toolkit","helpers","Click_me_to_upload_NGS_results_to_database.lnk"), os.path.join(args.out_folder, run_id))
		...
		#def generate_report(script_folder, report_folder, flatten_av, qc_dict, meta_dict, gene_exons_summary, failed_exons, alt_plot, base_plot, boxplot_paths_list):
		...
		generate_report(script_folder, report_folder, av_file, report_qc_dict[sample_id], meta_dict, sample_dict[sample_id]['gene_exons'], sample_dict[sample_id]['failed_exons'], alt_plot, base_plot, boxplots_list)
		
	print "Script finished successfully"
	print "Script run time: " + str(datetime.datetime.now() - start_time)
			
def checkRequisites(run_folder, align_folder):
...
def configure_analysis_by_sample_sheet(sampleSheetName, analysisTypeCSV, analysisSettingsFile):
	...
	for line in enumerate(lines):
		...
		if "Investigator Name," in line[1]:
			...
			for idx,sampleKey in enumerate(line[1].split(',')):
...
def filterBAM(align_folder, sample_id, qscore, coverage_output_suffix, bam_suffix, samtoolsCMD):
	...
	originalBAM = os.path.join(align_folder,sample_id + bam_suffix + ".bam")
	processedBAM = os.path.join(align_folder,sample_id + coverage_output_suffix + ".bam")
...
def get_coverage(in_folder, analysis_folder, sample_id, qscore, coverage, genelist, bed, rscript, coverage_output_suffix, bedtoolsCMD, rscriptCMD):
	...
	bamFile = os.path.join(in_folder,sample_id + coverage_output_suffix + ".bam")
	...
	[failed_exons_list, gene_exons_dict] = calcMetrics(histogramFile, coverage, genelist)
	...
	return [failed_exons_list, gene_exons_dict]
"""
=====================================
...
def calcMetrics(inHistFile, depthThresh, geneListFile):
	...
	for region in sorted(interval):
		...
		pctAboveThreshold = str(interval[region]['depthThresh'] / numBase * 100)
		pctZero = str(interval[region]['depthZero'] / numBase * 100)
		...
		if int(min) < int(depthThresh):
			...
			failed_exon = [filter,gene,accession,exon,chr,start,min,pctAboveThreshold]
...
def filter_av(av_in, av_out, genelist, bed, av_fields, variants_db):
	...
	for i, line in enumerate(infile):
		...
		if i == 0:
			...
			if not variantsDict:
				variantsDict = getVariantsInfo_sqlite(conn,chr,chrCoord,alt,ref,accession)
...
def report_qc_miseq(run_folder, run_id, sample_id):
...
def generate_report(script_folder, report_folder, flatten_av, qc_dict, meta_dict, genes_exons_summary, failed_exons, alt_plot, base_plot, boxplots_list):
	...
	report_pdf = os.path.join(report_folder, meta_dict['sample_id'] + ".pdf")
