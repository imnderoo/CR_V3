...
def main():
	...
	cnv_only_dict = get_cnv_only_dict(cnv_only_genelist)
		...
		with open(filter_av_file) as f:
			for row in f:
				for gene in cnv_only_dict:
					if gene not in row:
						if 'variants_called' not in sample_dict[sample_id]:
							sample_dict[sample_id]['variants_called'] = 1
						else:
							sample_dict[sample_id]['variants_called']  = sample_dict[sample_id]['variants_called']  + 1
	...
	for sample_id in sample_dict:
		...
		cnv_dict = parse_cnv(sample_id, cnv_folder, cnv_panel_csv, bed)
		...
		generate_report(script_folder, report_folder, av_file, cnv_dict, report_qc_dict[sample_id], meta_dict, sample_dict[sample_id]['gene_exons'], sample_dict[sample_id]['failed_exons'], cnv_only_dict, alt_plot, base_plot, boxplots_list, cnv_plots_list)
...
def get_cnv_only_dict(cnv_only_genelist):
	cnv_only_dict = {}
	
	for row in open(cnv_only_genelist, 'rb'):
		row_fields = row.rstrip('\r\n').split('\t')
		gene_name = row_fields[0]
		cnv_only_dict[gene_name] = row_fields[1]
	
	return cnv_only_dict
...
def parse_cnv(sample_id, cnv_folder, cnv_panel_csv_file, bed_file):
...
def generate_report(script_folder, report_folder, flatten_av, cnv_dict, qc_dict, meta_dict, genes_exons_summary, failed_exons, cnv_only_dict, alt_plot, base_plot, boxplots_list, cnv_plots_list):
