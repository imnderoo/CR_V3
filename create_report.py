...
def main():
	...
	#case_folder_root = "/media/sf_PLMGenetics/Case\\ Data/Molecular/Analytical\\ Data/" # If empty is. "", then this is bypassed and not used.
	case_folder_root = "/media/sf_PLMGenetics/Case Data/Molecular/Analytical Data/" # If empty is. "", then this is bypassed and not used.
	...
	for sample_id in sample_dict:
		...
		generate_report(script_folder, report_folder, case_folder_root, av_file, cnv_dict, report_qc_dict[sample_id], meta_dict, sample_dict[sample_id]['gene_exons'], sample_dict[sample_id]['failed_exons'], sample_dict[sample_id]['exons_summary_csv'], cnv_only_dict, alt_plot, base_plot, boxplots_list, cnv_plots_list)
...
def get_cnv_only_dict(cnv_only_genelist):
	...
	for row in open(cnv_only_genelist, 'rb'):
		...
		cnv_only_dict[gene_name] = accession
...
def generate_report(script_folder, report_folder, case_folder_root, flatten_av, cnv_dict, qc_dict, meta_dict, genes_exons_summary, failed_exons, exon_summary_csv, cnv_only_dict, alt_plot, base_plot, boxplots_list, cnv_plots_list):
	...
	if case_folder_root: 
		...
		if case_folder:
			...
			shutil.copy(report_pdf, case_folder)
				# Variant File - FLATTEN AV contains EPCAM variants. Need to filter out.
			shutil.copy(flatten_av, case_intermediates)
				#  QC File
			qc_file = open(os.path.join(case_intermediates, "qc_stats.csv"), 'w')
